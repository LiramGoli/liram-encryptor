...
public class ShiftMultiplyEncryption extends ShiftEncryption {
    ...
    protected String processCharacterEncryption(BigInteger value, Integer key) {
        return value.multiply(BigInteger.valueOf(key)).toString() + ENCRYPTED_CHARACTERS_SEPARATOR;
    }

    @Override
    protected String processCharacterDecryption(BigInteger value, Integer key) {
        return value.divide(BigInteger.valueOf(key)).toString() + ENCRYPTED_CHARACTERS_SEPARATOR;
    }
